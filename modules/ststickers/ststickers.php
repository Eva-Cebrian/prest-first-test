...
class StStickers extends Module implements WidgetInterface
{
	...
	public function getContent()
	{
        ...
        $this->_html .= '<script type="text/javascript">var googleFontsString=\''.json_encode($this->googleFonts).'\';var go_to_advanced_confirm="'.$this->getTranslator()->trans('Did you save all changes you just made? Unsaved changes will lose.', array(), 'Modules.Ststickers.Admin').'"</script>';
        ...
        if(Validate::isLoadedObject($sticker))
        {
            foreach ($languages as $lang)
                if($sticker->image_multi_lang[$lang['id_lang']])
                {
                    ...
                    '<p><a class="btn btn-default st_delete_image" data-id="'.$sticker->id.'" data-lang="'.(int)$lang['id_lang'].'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Panda').'</a></p>';
                }
        }
        ...
        if (!$this->isCached($this->templatePath.'header.tpl', $this->getCacheId()))
        {
            ...
            foreach ($custom_css_arr as $v) {
                ...
                if ($v['hide_on_mobile']==1) {
                    $custom_css .= '@media (max-width:991px){'.$prefix.'{display:none;}}';
                }elseif ($v['hide_on_mobile']==2) {
                    $custom_css .= '@media (min-width:992px){'.$prefix.'{display:none;}}';
                }
