...
class StProductComments extends Module implements WidgetInterface
{
        ...
        $this->_hooks = array(
            'Product' => array(
                ...
                array(
                    ...
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayProductDescRightColumn', array(), 'Admin.Theme.Panda'),
                ),
                array(
                    'id' => 'displayProductExtraContent',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayProductExtraContent(Product tabs)', array(), 'Admin.Theme.Panda'),
                ),
            ),
            'Hooks' => array(
                array(
    ...
    {
        $slider = __PS_BASE_URI__.'stupload/'.$slider;
    ...
    public function getContent()
	{
        ...
        if (Tools::isSubmit('add'.$this->name) || (Tools::isSubmit('update'.$this->name) && $id_st_product_comment))
        {
            ...
            $tabs = array();
            $tabs = array(
                array('id'  => '0,1,5,6,12,11', 'name' => $this->getTranslator()->trans('General settings', array(), 'Admin.Theme.Panda')),
        ...
        if (isset($_POST['save'.$this->name])) {
           ...
           if ($criterion->validateFields(false) && $criterion->save()) {
                ...
                foreach($form['form']['input'] as $field)
                    if(isset($field['validation']))
                    {
                        ...
                        if (isset($field['required']) && $field['required'] && $value==false && (string)$value != '0') {
                            $label = $field['label'];
                            $errors[] = $this->getTranslator()->trans('Field "%s%" is required.', array('%s%' => $label), 'Admin.Theme.Panda');
                        }	
                        elseif($value)
                        {
        					...
        					if (!Validate::$field_validation($value))
                                $errors[] = $this->getTranslator()->trans('Field "%s%" is invalid.', array('%s%' => $label), 'Admin.Theme.Panda');
                        }
        				// Set default value
            ...
            if (isset($_FILES['csv_file']) && $_FILES['csv_file'] && !empty($_FILES['csv_file']['tmp_name']) && $_FILES['csv_file']['error'] == 0) {
                ...
                if ($ext != 'csv') {
                    $this->validation_errors[] = $this->getTranslator()->trans('Please upload csv file.', array(), 'Modules.Stproductcomments.Admin');
                }
                $file = _PS_MODULE_DIR_.$this->name.'/import/'.date('YmdH').'.csv';
                if (!move_uploaded_file($_FILES['csv_file']['tmp_name'], $file)) {
                    $this->validation_errors[] = $this->getTranslator()->trans('Move upload file failed.', array(), 'Modules.Stproductcomments.Admin');
                }
                if (file_exists($file)) {
                    if ($fp = fopen($file, 'r')) {
                        set_time_limit(6*60);
                    ...
                    $this->validation_errors[] = $this->getTranslator()->trans('Upload file failed, plase ensure the folder %1% is writable.', array('%1%' => dirname($file)), 'Modules.Stproductcomments.Admin');
                }
            }
    ...
    protected function initConfigFormFields()
    {
        ...
        $this->fields_form[11]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Comment list page', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
            ),
            'input' => $fields['list'],
            'submit' => array(
        ...
        $this->fields_form[12]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Import comments from CSV file', array(), 'Modules.Stproductcomments.Admin'),
                'icon' => 'icon-cogs'
            ),
            'input' => array(
                array(
                    'type' => 'file',
                    'label' => $this->getTranslator()->trans('Upload CSV file:', array(), 'Modules.Stproductcomments.Admin'),
                    'name' => 'csv_file',
                    'desc' => array(
                        $this->getTranslator()->trans('Download CSV template %1% here %2%', array('%1%' => '<a href="'._MODULE_DIR_.$this->name.'/sample_comment.csv">', '%2%' => '</a>'), 'Modules.Stproductcomments.Admin'),
                    ),
                ),
            ),
            'submit' => array(
    ...
    public function renderCommentForm()
	{
            ...
            if($images) {
                ...
                foreach($images as $image) {
                    $pc_images['data'][] = array(
                        'name' => $image,
                        'url' => context::getContext()->link->protocol_content.Tools::getMediaServer(__PS_BASE_URI__.'stupload/'.$image).__PS_BASE_URI__.'stupload/'.$image,
                        );
                }
            }
        }
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                'pc_images' => array(
                    'type' => 'pc_images',
                    'label' => $this->trans('Images:', array(), 'Admin.Theme.Panda'),
                    'name' => 'image',
                    'values' => $pc_images,
                 ),
                 array(
                ...
                'date_add' => array(
                    'type' => 'date',
                    'label' => $this->trans('Created on:', array(), 'Admin.Theme.Panda'),
                    'name' => 'date_add',
    ...
    public function _prepareHook($ext='')
    {
        ...
        if($rewind_nav && (int)Configuration::get($this->_prefix_st.'PRO_PER_FW')>count($pcomments)){
            $rewind_nav=false;
        }
        $this->smarty->assign(array(
            'pcomments'              => $pcomments,
    ...
    public function hookDisplayHeader($params)
    {
        ...
        Media::addJsDef(array(
            ...
            'confirm_report_message' => $this->getTranslator()->trans('Are you sure report abuse ?', array(), 'Shop.Theme.Panda'),
            'dictRemoveFile' => $this->getTranslator()->trans('Remove', array(), 'Shop.Theme.Panda'),
            'st_pc_display_format' => $this->getTranslator()->trans('#left / #max Characters left.', array(), 'Shop.Theme.Panda'),
    ...
    public function hookActionExportGDPRData($customer)
    {
        if (!Tools::isEmpty($customer['id'])) {
            ...
            return json_encode($this->getTranslator()->trans('Product comments: No data found.', array(), 'Shop.Theme.Panda'));
       }
   }
