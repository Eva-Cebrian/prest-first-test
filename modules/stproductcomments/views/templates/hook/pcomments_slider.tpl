...
<div id="stproductcomments_container_{$hook_hash}" class="stproductcomments_container {if $hide_mob} hidden-xs {/if} block {if !$column_fix && ($has_background_img || $video_mpfour)} jarallax {/if} products_container{$column_fix} {if $column_slider && !isset($is_quarter)} column_block {/if}" 
