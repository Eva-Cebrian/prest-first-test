...
class StProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	protected function ajaxProcessAddComment()
	{
		...
		if (!Validate::isInt(Tools::getValue('id_product')))
			$errors[] = $this->trans('Invalid Product', array(), 'Shop.Theme.Panda');
		...
		if (!count($errors))
		{
			...
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + Configuration::get($this->_prefix_st.'MINIMAL_TIME')) < time()))
			{
                ...
                if ($comment->id && Configuration::get($this->_prefix_st.'NOTICE_WEBMASTER')) {
                    ...
                    Mail::Send(
                        ...
                        'review_admin',
                        $this->trans('New comment for product', array(), 'Shop.Theme.Panda').'[#'.$comment->id_proudct.']',
                        $param,
                        $email,
                        null,
                        null,
			...
			else
			{
                ...
                $result['m'] = $this->trans('You should wait %seconds% seconds before posting a new comment.', array('%seconds%'=>$minimal_time), 'Shop.Theme.Panda');
			}
		}
		else
    ...
    protected function ajaxProcessAddReply()
	{
		...
		if (!$comment->id)
			$errors[] = $this->trans('Invalid parameter ', array(), 'Shop.Theme.Panda');
		if (!Tools::getValue('content') || !Validate::isMessage(Tools::getValue('content')))
			$errors[] = $this->trans('Your comment is too short. Please try again', array(), 'Shop.Theme.Panda');
        $customer_name = Tools::getValue('customer_name');
        if (!Validate::isGenericName($customer_name))
            $errors[] = $this->trans('Invalid name', array(), 'Shop.Theme.Panda');
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
                   ...
                   if(!ImageManager::resize($temp_name, _PS_ROOT_DIR_.'/stupload/'.'stproductcomments/'.$c_name.'.'.$type, null, null, $type))
				       $result['error'] = $this->trans('An error occurred during the image upload.', array(), 'Shop.Theme.Panda');
				} 
				if (isset($temp_name))
					@unlink($temp_name);
