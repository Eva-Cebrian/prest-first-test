...
class StNewsLetter extends Module implements WidgetInterface
{
    ...
    private function initPages()
    {
        $this->_pages = array(
                ...
                array(
                    ...
                    'val' => '1024',
                    'name' => $this->getTranslator()->trans('Blog homepage', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'module-stblog-category',
                    'val' => '2048',
                    'name' => $this->getTranslator()->trans('Blog category', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'module-stblog-article',
                    'val' => '4096',
                    'name' => $this->getTranslator()->trans('Blog post', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'authentication',
                    'val' => '8192',
                    'name' => $this->getTranslator()->trans('Login and registration', array(), 'Admin.Theme.Panda')
                ),
            );
    }
    ...
    public function installDB()
    {
        $return = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_news_letter` (
                ...
                `showonclick` tinyint(1) unsigned NOT NULL DEFAULT 0,
    ...
    public function getContent()
    {
        ...
        if ((Tools::isSubmit('groupdeleteimagestnewsletter')))
        {
            ...
            if($news_letter->id)
            {
                @unlink(_PS_ROOT_DIR_.__PS_BASE_URI__.'stupload/'.$this->name.'/'.$news_letter->bg_img);
                $news_letter->bg_img = '';
                if ($news_letter->save())
                {
            ...
            if(!count($error))
            {
                if (isset($_FILES['bg_img']) && isset($_FILES['bg_img']['tmp_name']) && !empty($_FILES['bg_img']['tmp_name'])) 
                {
                    ...
                    else 
                    {
                        ...
                        if (!move_uploaded_file($_FILES['bg_img']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$bg_image))
    ...
    public function uploadCheckAndGetName($name)
    {
        ...
        while (file_exists(_PS_ROOT_DIR_.'/stupload/'.$filename.'.'.$type)) {
            $filename .= rand(10, 99);
        } 
    ...
    public static function showApplyTo($value,$row)
    {
        if(isset(self::$location[$value]))
            ...
            return Module::isInstalled('jscomposer') && Module::isEnabled('jscomposer') ? $context->getTranslator()->trans('For visual composer module', array(), 'Admin.Theme.Panda') : $context->getTranslator()->trans('--', array(), 'Admin.Theme.Panda');
        }
    }
    ...
    {        
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('On mobile footer:', array(), 'Admin.Theme.Panda'),
                    'name' => 'hide_on_mobile',
                    'default_value' => 0,
                    'values' => array(
                        array(
        ...
        $this->fields_form[2]['form'] = array(
            ...
            'input' => array( 
                ...
                'showonclick' => array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Show the popup when a button is clicked:', array(), 'Modules.Stnewsletter.Admin'),
                    'name' => 'showonclick',
                    'is_bool' => true,
                    'default_value' => 1,
                    'values' => array(
                    ...
                    'desc' => array(
                        $this->getTranslator()->trans('The popup won\'t show out, till a button is clicked.', array(), 'Modules.Stnewsletter.Admin'),
                    ),
                ),
                array(
                    'type' => 'switch',
        ...
        if($news_letter->id)
        {
            ...
            $this->fields_form[2]['form']['input']['showonclick']['desc'][] = $this->getTranslator()->trans('Put #st_news_letter_popup_%s% to the link field of a button, the popup will be shown out when the button is clicked.', array('%s%'=>$news_letter->id), 'Modules.Stnewsletter.Admin');
        }else{
            $this->fields_form[2]['form']['input']['showonclick']['desc'][] = $this->getTranslator()->trans('Save this record first to see the instraction of how to use the feautre.', array(), 'Modules.Stnewsletter.Admin');
        }
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'desc' => $this->getTranslator()->trans('Create a voucher code for each subscriber base on given voucher code.', array(), 'Admin.Theme.Panda'),
                    'validation' => 'isBool',
                ),
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Send vouchers to registered users only', array(), 'Modules.Stnewsletter.Admin'),
                    'name' => 'user_only',
                    'values' => array(
                        array(
                            'id' => 'user_only_on',
                    ...
                    'desc' => $this->getTranslator()->trans('Every user will get a voucher assigned under their accounts.', array(), 'Admin.Theme.Panda'),
                    'validation' => 'isBool',
                ),
                array(
                    'type' => 'text',
                    ...
                    'suffix' => 'days',
                    'class' => 'fixed-width-md',
                    'desc' => $this->getTranslator()->trans('How many days the voucher code is valid.', array(), 'Admin.Theme.Panda'),
                    'validation' => 'isString',
                ),
                array(
                    'type' => 'textarea',
    ...
    public function hookDisplayHeader($params)
    {
        Media::addJsDef(array(
            'wrongemailaddress_stnewsletter' => $this->getTranslator()->trans('Invalid email address', array(), 'Shop.Theme.Panda'),
        ));
        ...
        if (!$this->isCached($this->templatePath.'header.tpl', $this->getCacheId()))
        {
            ...
            if(is_array($options) && count($options))
                foreach($options as $v)    
                {
                    ...
                    if ($v['bg_img'])
                    {
                        $img = __PS_BASE_URI__.'stupload/'.$v['bg_img'];
    ...
    */
        ...
        if(is_array($news_letter_array) && count($news_letter_array))
            foreach($news_letter_array as $k => &$v)
            {
                if($v['location']==4)
                {
                    ...
                    if ($v['subscribed'] && isset($this->context->cookie->email) && Db::getInstance()->getValue('SELECT count(0) FROM `'._DB_PREFIX_.'emailsubscription` WHERE `email` = \''.$this->context->cookie->email.'\''))
                    {
                        unset($news_letter_array[$k]);
                        continue;
                    }
	...
	public function newsletterRegistration()
	{
		...
		if (empty($email) || !Validate::isEmail($email))
			$error = $this->getTranslator()->trans('Invalid email address.', array(), 'Shop.Theme.Panda');
		/* Unsubscription */
		else if ($action == '1')
		{
			...
			if ($register_status < 1)
				$error = $this->getTranslator()->trans('This email address is not registered.', array(), 'Shop.Theme.Panda');
			elseif (!$newsletterClass->unregister($email, $register_status))
				$error = $this->getTranslator()->trans('An error occurred while attempting to unsubscribe.', array(), 'Shop.Theme.Panda');
			else
                $valid = $this->getTranslator()->trans('Unsubscription successful.', array(), 'Shop.Theme.Panda');
		...
		else if ($action == '0')
		{
            ...
            if ($register_status > 0)
                $error = $this->getTranslator()->trans('This email address is already registered.', array(), 'Shop.Theme.Panda');
			...
			if (!$error && !$newsletterClass->isRegistered($register_status))
			{
				if (Configuration::get('ST_NW_VERIFICATION_EMAIL'))
				{
					...
					if (!$token = $newsletterClass->getToken($email, $register_status)) {
					   $error = $this->getTranslator()->trans('An error occurred during the subscription process.', array(), 'Shop.Theme.Panda');
					}
                    
                    $this->sendVerificationEmail($email, $token);

					$valid = $this->getTranslator()->trans('A verification email has been sent. Please check your inbox.', array(), 'Shop.Theme.Panda');
				}
				else
				{
					if ($newsletterClass->register($email, $register_status))
						$valid = $this->getTranslator()->trans('You have successfully subscribed to this newsletter.', array(), 'Shop.Theme.Panda');
					else
						$error = $this->getTranslator()->trans('An error occurred during the subscription process.', array(), 'Shop.Theme.Panda');
	...
	public function confirmEmail($token)
	{
		...
		if (!$activated)
			return $this->getTranslator()->trans('This email is already registered and/or invalid.', array(), 'Shop.Theme.Panda');
		...
		return $this->getTranslator()->trans('Thank you for subscribing to our newsletter.', array(), 'Shop.Theme.Panda');
	}
    ...
    private function getConditionFixtures($lang)
    {
        ...
        return
            $this->trans('You may unsubscribe at any moment. For that purpose, please find our contact info in the legal notice.', array(), 'Shop.Theme.Panda')
        ;
    }
    ...
    function vc_map_init()
    {
        ...
        foreach(StNewsLetterClass::getNewsLetter($this->context->language->id, 0, 1) AS $value) {
            $content[$this->getTranslator()->trans('ID:', array(), 'Admin.Theme.v').$value['id_st_news_letter']] = $value['id_st_news_letter'];
    ...
    public function hookActionDeleteGDPRCustomer($customer)
    {
        if (!empty($customer['email']) && Validate::isEmail($customer['email'])) {
            ...
            return json_encode($this->getTranslator()->trans('Newsletter subscription: Unable to delete customer using email.', array(), 'Shop.Theme.Panda'));
        }
    }
    public function hookActionExportGDPRData($customer)
    {
        if (!Tools::isEmpty($customer['email']) && Validate::isEmail($customer['email'])) {
            ...
            return json_encode($this->getTranslator()->trans('Newsletter subscription: No data found.', array(), 'Shop.Theme.Panda'));
       }
   }
}