...
class StNewsLetterClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields'    => array(
            ...
            'showonclick'         =>array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
            // Lang fields
        ...
        foreach($fields AS $field)
        {
            if (is_array($slider) && isset($slider[$field]) && $slider[$field])
            {
                if(strpos($slider[$field], '/modules/') === false)
                    $slider[$field] = __PS_BASE_URI__.'stupload/'.$slider[$field];
    ...
    {
        $sql = 'INSERT INTO '._DB_PREFIX_.'emailsubscription (id_shop, id_shop_group, email, newsletter_date_add, ip_registration_newsletter, http_referer, active, id_lang, id_gender)
                VALUES
