...
class StPageBanner extends Module implements WidgetInterface
{
	...
	public function getContent()
	{
        ...
        $this->_html .= '<script type="text/javascript">var googleFontsString=\''.json_encode($this->googleFonts).'\';</script>';
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
                   ...
                   if(!ImageManager::resize($temp_name, _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$c_name.'.'.$type, null, null, $type))
				       $result['error'][] = $this->getTranslator()->trans('An error occurred during the image upload.', array(), 'Admin.Theme.Panda');
				} 
				if (isset($temp_name))
					@unlink($temp_name);
	...
	{        
		...
		$this->fields_form_banner[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Show on subcategories:', array(), 'Admin.Theme.Panda'),
                    'name' => 'show_subcate',
                    'default_value' => 0,
                    'is_bool' => true,
                    'values' => array(
                    ...
                    'desc' => $this->getTranslator()->trans('Show on sub categories, just for category only.', array(), 'Admin.Theme.Panda'),
                ),
                array(
                    'type' => 'text',
        ...
        $this->fields_form_banner[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                 ...
                 array(
                    'type' => 'textarea',
                    'label' => $this->getTranslator()->trans('Custom content:', array(), 'Admin.Theme.Panda'),
                    'lang' => true,
                    'name' => 'description',
                    'cols' => 40,
                    'rows' => 10,
    ...
    {
        return array(
            'authentication' => array('id'=>'10_authentication', 'name'=>$this->getTranslator()->trans('Login', array(), 'Shop.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Login', array(), 'Shop.Theme.Panda')),
    ...
    public function getInformationLinks()
    {
        return array(
            ...
            'checkout' => array('id'=>'10_checkout', 'name'=>$this->getTranslator()->trans('Checkout', array(), 'Shop.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Checkout', array(), 'Shop.Theme.Panda'), 'php_self'=>'order'),
            'order-confirmation' => array('id'=>'10_order-confirmation', 'name'=>$this->getTranslator()->trans('Order confirmation', array(), 'Shop.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Order confirmation', array(), 'Shop.Theme.Panda')),
            'search' => array('id'=>'10_search', 'name'=>$this->getTranslator()->trans('Search', array(), 'Shop.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Search', array(), 'Shop.Theme.Panda')),
            'cart' => array('id'=>'10_cart', 'name'=>$this->getTranslator()->trans('Shopping cart', array(), 'Shop.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Shopping cart', array(), 'Shop.Theme.Panda')),           
        );
    }
    ...
    public function hookDisplayBreadcrumb($params)
    {
        ...
        {
            if ($page_name == 'module-amazzingblog-blog' && isset($this->context->controller->ab_data))
                $id = $this->context->controller->ab_data['type'].'-'.$this->context->controller->ab_data['id'];
            elseif ($page_name == 'module-af_seopages-seopage' && isset($this->context->controller->seopage_data))
                $id = $this->context->controller->seopage_data['id_seopage'];
