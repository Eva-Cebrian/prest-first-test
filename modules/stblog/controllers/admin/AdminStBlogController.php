...
class AdminStBlogController extends ModuleAdminController
{
	...
	public function setMedia($isNewTheme = false)
	{
		...
		if (!file_exists(_PS_BO_ALL_THEMES_DIR_.$bo_theme.DIRECTORY_SEPARATOR
			.'template'))
			$bo_theme = 'default';
        $this->addCss(_MODULE_DIR_.'stblog/views/css/admin.css');
        $this->addJs(_MODULE_DIR_.'stblog/views/js/admin_blog.js');
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAdd'.$this->table) || Tools::isSubmit('submitAdd'.$this->table.'AndStay'))
		{
            ...
            // Use exact time to calc timeago
        ...
        foreach ($languages as $lang)
        {
            ...
            if ($this->object->id) {
                ...
                $desc = $img ? '<img src="'.$img.'" width="200"/>'.
                    '<p><a class="btn btn-default st_blog_delete_cover" data-id="'.$this->object->id.'" data-lang="'.(int)$lang['id_lang'].'" data-token="'.Tools::getValue('token').'" href="javascript:;"><i class="icon-trash"></i> '.$this->trans(' Delete', array(), 'Admin.Theme.Panda').'</a></p>' : '';
            }
   	...
   	public function ajaxProcessDeleteGalleryImage()
    {
        ...
        if (!($id_image = (int)Tools::getValue('id_image')))
            die(json_encode(array('error'=>$this->l('An error occurred (the image not exists).'),'id'=>$id_image)));
        
        $image = new StBlogImageClass('2', $id_image);
        
        if (!$image->id)
            die(json_encode(array('error'=>$this->l('An error occurred (unknown object).'),'id'=>$id_image)));
        
        if ($image->setShopList($image->getAssociatedShops())->delete())
        {
            $image->refreshPositions();
            die(json_encode(array('success'=>$this->l('The picture was deleted successful.'),'id'=>$id_image)));
        }
        else
            die(json_encode(array('error'=>$this->l('The picture was deleted successful.'),'id'=>$id_image)));
    }
