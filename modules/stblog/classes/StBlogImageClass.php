...
define('_PS_ST_BLOG_IMG_DIR_', _PS_ROOT_DIR_.'/stupload/'.'stblog/');
...
class StBlogImageClass extends ObjectModel
{
       ...
       if (!is_file($tmp_file))
        ...
        foreach($this->getImageTypes() AS $key => $type)
        {
            ...
            /*if (Module::isEnabled('stwebp') && method_exists('ImageManager', 'cut')) {
                $ret &= ImageManager::cut($tmp_file, $folder.$file.$key.'.'.$ext, $width, $height);
                continue;
            }*/
            ...
            ImageManager::resize($tmp_file_new, $folder.$file.$key.'.'.$ext, $width, $height);
    ...
    public static function getImageLinks($image,$type=null)
	{
	    ...
	    $upload_folder = __PS_BASE_URI__.'stupload/'.'stblog/';
    ...
    /** 
      ...
      */
    public static function getImageIdByType($id_st_blog = 0, $type = '1', $id_lang=null)
    {
        $ret = array();
        if (!$id_st_blog || !$type)
            return $ret;
