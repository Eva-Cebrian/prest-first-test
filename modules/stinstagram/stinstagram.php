...
*/
    ...
    private function _checkImageDir()
    {
        ...
        if (!file_exists(_PS_ROOT_DIR_.'/stupload/'.$this->name)) {
            $result = @mkdir(_PS_ROOT_DIR_.'/stupload/'.$this->name, self::$access_rights, true)
                        || @chmod(_PS_ROOT_DIR_.'/stupload/'.$this->name, self::$access_rights);
            if (!$result) {
                $this->_html .= $this->displayError('"'._PS_ROOT_DIR_.'/stupload/'.$this->name.'" '.$this->getTranslator()->trans('An error occurred during new folder creation', array(), 'Modules.Stinstagram.Admin'));
            } else {
        ...
        if (!is_writable(_PS_ROOT_DIR_.'/stupload/')) {
            $result = false;
            $this->_html .= $this->displayError('"'._PS_ROOT_DIR_.'/stupload/'.$this->name.'" '.$this->getTranslator()->trans('directory isn\'t writable.', array(), 'Modules.Stinstagram.Admin'));
        }
