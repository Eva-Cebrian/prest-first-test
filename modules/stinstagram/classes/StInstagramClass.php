...
class StInstagramClass extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields'    => array(
            ...
            'pause_on_enter'            => array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'reverse_direction'            => array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
        ...
        foreach ($fields as $field) {
            if (is_array($image) && tools::getIsset($image[$field]) && $image[$field]) {
                if (strpos($image[$field], '/upload/') === false && strpos($image[$field], '/modules/') === false) {
                    $image[$field] = __PS_BASE_URI__.'stupload/'.$image[$field];
                }
    ...
    public function tongbu($token=false,$userid=false,$st=0){
        ...
        if(isset($tmpUserData['error'])){
            if((Tools::getValue('act')=='update_cache_img' || Tools::getValue('act')=='clear_cache_img')){
                die(json_encode(['state'=>0,'msg'=>$tmpUserData['error']['message']]));
            }else{
                return false;
            }
        }
