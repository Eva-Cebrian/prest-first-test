...
class StSocial extends Module implements WidgetInterface
{
	...
	public function getContent()
	{
        ...
        if(Tools::getValue('act')=='delete_image')
        {
            ...
            if(Configuration::updateValue($this->_prefix_st.'LOGO_IMG', '')) {
                $result['r'] = true;
                @unlink(_PS_ROOT_DIR_.'/stupload/'.$img);
            } 
            ...
            if(!count($this->validation_errors))
            {
                if (isset($_FILES['logo_img']) && isset($_FILES['logo_img']['tmp_name']) && !empty($_FILES['logo_img']['tmp_name'])) 
                {
                    ...
                    else 
                    {
                        ...
                        if (!move_uploaded_file($_FILES['logo_img']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$logo_image))
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Description:', array(), 'Admin.Theme.Panda'),
                    'name' => 'description',
                    'lang' => true,
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('New window:', array(), 'Admin.Theme.Panda'),
                    'name' => 'new_window',
                    'is_bool' => true,
                    'default_value' => 1,
                    'values' => array(
                ...
                array(
                    ...
                    'values' => array(
                        array(
                            'id' => 'links_size_0',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Small', array(), 'Admin.Theme.Panda')),
                        array(
                            'id' => 'links_size_1',
                            'value' => 1,
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if ($has_facebook) {
            ...
            {
                $img = _PS_ROOT_DIR_.__PS_BASE_URI__.'stupload/'.$logo_image;
                $this->assignImageDimension($img);
                $this->smarty->assign(array('logo_image_link' => _PS_BASE_URL_.__PS_BASE_URI__.'stupload/'.$logo_image));
            }
