...
class StWishlist extends Module implements WidgetInterface
{
	...
	public function install()
	{
		$res = parent::install()
            ...
            && Configuration::updateValue($this->_prefix_st.'SHOW_ICON', 0)
            ;
        if ($res)
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[6]['form'] = array(
            ...
            'input' => array(
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('How to display:', array(), 'Modules.Stwishlist.Admin'),
                    'name' => 'show_icon',
                    'default_value' => 0,
                    'values' => array(
                        array(
                            'id' => 'show_icon_0',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Display the whishlist icon out when mouse hover.', array(), 'Admin.Theme.Panda')),
                        array(
                            'id' => 'show_icon_1',
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Always display the whishlist icon.', array(), 'Admin.Theme.Panda')),
                    ),
                    'validation' => 'isUnsignedInt',
                ),
                array(
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if (!$this->isCached($this->templatePath.'header.tpl', $this->getCacheId()))
        {
            ...
            $prefix = '.add_to_wishlist.layer_btn';
            ...
            if($text_color = Configuration::get($this->_prefix_st.'TEXT_COLOR'))
                $custom_css .= '.add_to_wishlist, .add_to_wishlist:hover{color:'.$text_color.';}';
            if($text_hover_color = Configuration::get($this->_prefix_st.'TEXT_HOVER_COLOR'))
                $custom_css .= '.add_to_wishlist.st_added, .add_to_wishlist.st_added:hover{color:'.$text_hover_color.';}';
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        return array(
            ...
            'wishlist_show_icon'     => Configuration::get($this->_prefix_st.'SHOW_ICON'),
        );        
    }
    ...
    private function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'product_style'        => Configuration::get($this->_prefix_st.'PRODUCT_STYLE'),
            'show_icon'      => Configuration::get($this->_prefix_st.'SHOW_ICON'),
        );
        foreach($this->_hooks AS $key => $values)
        {
            if (!$key)
