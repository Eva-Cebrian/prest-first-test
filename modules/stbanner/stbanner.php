...
class StBanner extends Module implements WidgetInterface
{
    ...
    public function sampleData($id_shop)
    {
		...
		foreach($samples as $k=>&$sample)
		{
            ...
            $module->width_md = $sample['width_md'];
    ...
    private function initPages()
    {
        $this->_pages = array(
                ...
                array(
                    'id' => 'search',
                    'val' => '512',
                    'name' => $this->getTranslator()->trans('Search', array(), 'Admin.Theme.Panda')
                ),
                array(
                ...
                array(
                    ...
                    'val' => '2048',
                    'name' => $this->getTranslator()->trans('Blog article', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'module-stblogarchives-default',
                    'val' => '4096',
                    'name' => $this->getTranslator()->trans('Blog archives', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'module-stblogsearch-default',
                    'val' => '8192',
                    'name' => $this->getTranslator()->trans('Blog search', array(), 'Admin.Theme.Panda')
                ),
            );
    }
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
                   ...
                   if($type == 'gif' && !copy($temp_name, _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$c_name.'.'.$type)) {
				       $result['error'][] = $this->getTranslator()->trans('An error occurred during the image upload.', array(), 'Admin.Theme.Panda');
                   } elseif ($type != 'gif' && !ImageManager::resize($temp_name, _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$c_name.'.'.$type, null, null, $type)) {
                        $result['error'][] = $this->getTranslator()->trans('An error occurred during the image upload.', array(), 'Admin.Theme.Panda');
                   }
				} 
				if (isset($temp_name))
	...
	protected function initFormColumn()
	{
		...
		$this->fields_form_column[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Width on desktop (screen width > 768px):', array(), 'Admin.Theme.Panda'),
                    'name' => 'width',
                    'options' => array(
                        'query' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Width on tablet (screen width < 768px and >480px):', array(), 'Admin.Theme.Panda'),
                    'name' => 'width_md',
                    'options' => array(
                        'query' => array(
                                array('id'=>1, 'name'=> '1/12'),
		...
		$this->fields_form_banner[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'size' => 64,
                    'lang' => true,
                    'desc' => '<strong>'.$this->getTranslator()->trans('If the link field is filled, then don\'t add links or buttons in the Caption field below - it will break the layout.', array(), 'Modules.Stbanner.Admin').'</strong>',
                ),
                array(
                    'type' => 'switch',
        ...
        foreach (self::$media_query as $mqk=>$mqv){
            foreach ($languages as $lang)
            {
                $this->fields_form_banner[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']] = array(
                        'type' => 'file',
    					'label' => $this->getTranslator()->trans('Image', array(), 'Admin.Theme.Panda').($mqk ? ' '.$this->getTranslator()->trans('for screen width<%1%px', array('%1%'=>$mqk), 'Admin.Theme.Panda') : '').' - '.$lang['name'].($default_lang == $lang['id_lang'] ? '('.$this->gettranslator()->trans('default language', array(), 'Admin.Theme.Panda').')' : '').':',
    ...
    function vc_map_init()
    {
        ...
        foreach(StBannerGroup::getBannerGroup(0, 100) AS $value) {
            $banners[$this->getTranslator()->trans('ID:', array(), 'Admin.Theme.Panda').$value['id_st_banner_group'].'|'.$value['name']] = $value['id_st_banner_group'];
