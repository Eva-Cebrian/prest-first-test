...
class ThemeLicense
{
	...
	public function GoumaimaIsValid()
	{
		...
		if (!$this->isRegistered()) {
			...
			if ((!(bool)Configuration::get('PS_SHOP_ENABLE') && !(bool)Configuration::get('PS_ENABLE_SHOP')) || strpos($shop->domain, 'localhost') !== false || strpos($shop->domain, '127.0.0.1') !== false || strpos($shop->domain, '192.168') !== false) {
				return true;
			}
			return;
		}
	...
	public function getTheme($version = true)
	{
		...
		$theme = $context->shop->theme->get('parent') ? $context->shop->theme->get('parent') : $context->shop->theme->getName();
    ...
    public function checkFiles()
    {
    	...
    	$file_name = 'checksum-'.$theme.'-'.$this->themeeditor->version.'.xml';
        $xml_remotefile = $this->dl_api_url.'/downloads/'.$theme.'/xml/'.$file_name;
