...
{	
    ...
    public function uploadCheckAndGetName($name)
    {
		...
		while (file_exists(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$filename.'.'.$type)) {
            $filename .= rand(10, 99);
        } 
    ...
    private function _checkImageDir($dir)
    {
        ...
        if (!file_exists($dir))
        {
            ...
            if(!$success)
                ...
                @copy(_PS_UPLOAD_DIR_.'.htaccess', _PS_ROOT_DIR_.'/stupload/.htaccess');
        }
	...
	public function getContent()
	{
        ...
        if(Tools::getValue('act')=='delete_image' && $field=Tools::getValue('field'))
        {
        	...
        	if (Configuration::get('STSN_'.$field))
                if (file_exists(_PS_ROOT_DIR_.'/stupload/'.$image))
                    @unlink(_PS_ROOT_DIR_.'/stupload/'.$image);
        ...
        if(isset($_POST['savestthemeeditor']))
		{
            ...
            $this->_checkImageDir(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/');
            ...
            foreach($bg_array as $v)
            {
    			if (isset($_FILES[$v.'_bg_image_field']) && isset($_FILES[$v.'_bg_image_field']['tmp_name']) && !empty($_FILES[$v.'_bg_image_field']['tmp_name'])) 
                {
                    ...
                    else 
                    {
    					...
    					if (!move_uploaded_file($_FILES[$v.'_bg_image_field']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$footer_image))
            ...
            if (isset($_FILES['footer_image_field']) && isset($_FILES['footer_image_field']['tmp_name']) && !empty($_FILES['footer_image_field']['tmp_name'])) 
            {
                ...
                else 
                {
                    ...
                    if(!$footer_image)
                        $this->validation_errors[] = $this->l('Image format not recognized');
                    else if (!move_uploaded_file($_FILES['footer_image_field']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$footer_image))
                        $this->validation_errors[] = $this->l('Error move uploaded file');
                    else
                    {
            ...
            if (isset($_FILES['retina_logo_image_field']) && isset($_FILES['retina_logo_image_field']['tmp_name']) && !empty($_FILES['retina_logo_image_field']['tmp_name'])) 
            {
                ...
                else 
                {
                    ...
                    if(!$retina_logo)
                        $this->validation_errors[] = $this->l('Image format not recognized');
                    else if (!move_uploaded_file($_FILES['retina_logo_image_field']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$retina_logo))
                        $this->validation_errors[] = $this->l('Error move uploaded file');
                    else
                    {
            ...
            if (isset($_FILES['mobile_logo_image_field']) && isset($_FILES['mobile_logo_image_field']['tmp_name']) && !empty($_FILES['mobile_logo_image_field']['tmp_name'])) 
            {
                ...
                if (!empty($imagesize)){
                    ...
                    else 
                    {
                        ...
                        if(!$mobile_logo)
                            $this->validation_errors[] = $this->l('Image format not recognized');
                        else if (!move_uploaded_file($_FILES['mobile_logo_image_field']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$mobile_logo))
                            $this->validation_errors[] = $this->l('Error move uploaded file');
                        else
                        {
            ...
            foreach($iphone_icon_array as $v)
            {
        			if (isset($_FILES['icon_iphone_'.$v.'_field']) && isset($_FILES['icon_iphone_'.$v.'_field']['tmp_name']) && !empty($_FILES['icon_iphone_'.$v.'_field']['tmp_name'])) 
                    {
                        ...
                        else 
                        {
                            ...
                            if(!$icon)
                                $this->validation_errors[] = $this->l('Image format not recognized');
                            else if (!move_uploaded_file($_FILES['icon_iphone_'.$v.'_field']['tmp_name'], _PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$icon))
                                $this->validation_errors[] = $this->l('Error move uploaded file');
                            else
                            {
        ...
        if (Tools::isSubmit('deleteimagestthemeeditor'))
        {
            if($identi = Tools::getValue('identi'))
            {
            	...
            	if (Configuration::get('STSN_'.$identi))
                    if (file_exists(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$image))
		                @unlink(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$image);
    ...
    public function generateBrowserConfig($favicon_update_time){
        if($favicon_update_time){
            if($browserconfig_icon = Configuration::get('STSN_ICON_IPHONE_150')){
                ...
                $this->_checkImageDir(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/');
                @file_put_contents(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/browserconfig.xml', $browserconfig);
            }
    ...
    public function generateWebmanifest($favicon_update_time){
        if($favicon_update_time){
            ...
            if($icon_iphone_192 || $icon_iphone_512){
                ...
                $this->_checkImageDir(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/');
                @file_put_contents(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/site.webmanifest', json_encode($site_webmanifest));
            }
            Configuration::updateValue('STSN_FAVICON_UPDATE_TIME', $favicon_update_time);
        }
        return true;
    ...
    public function initForm()
    {
		...
		if ($footer_img != "") {
            $this->fields_form[0]['form']['input']['payment_icon']['image'] = $this->getImageHtml((strpos($footer_img, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : ($footer_img==$this->defaults["footer_img"]['val'] ? $this->_path : __PS_BASE_URI__.'stupload/' )).$footer_img,'footer_img');
		}
        $retina_logo = Configuration::get('STSN_RETINA_LOGO');
        if ($retina_logo != "") {
            $this->fields_form[4]['form']['input']['retina_logo_image_field']['image'] = $this->getImageHtml((strpos($retina_logo, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : __PS_BASE_URI__.'stupload/').$retina_logo,'retina_logo');
        }
        $mobile_logo = Configuration::get('STSN_MOBILE_LOGO');
        if ($mobile_logo != "") {
            $this->fields_form[4]['form']['input']['mobile_logo_image_field']['image'] = $this->getImageHtml((strpos($mobile_logo, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : __PS_BASE_URI__.'stupload/').$mobile_logo,'mobile_logo');
        }
		/*if (Configuration::get('STSN_ICON_IPHONE_57') != "") {
            $iphone_57 = Configuration::get('STSN_ICON_IPHONE_57');
		    $this->fields_form[18]['form']['input']['icon_iphone_57_field']['image'] = $this->getImageHtml(($iphone_57 != $this->defaults['icon_iphone_57']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).$iphone_57,'icon_iphone_57');
		}
		if (Configuration::get('STSN_ICON_IPHONE_72') != "") {
            $iphone_72 = Configuration::get('STSN_ICON_IPHONE_72');
		    $this->fields_form[18]['form']['input']['icon_iphone_72_field']['image'] = $this->getImageHtml(($iphone_72 != $this->defaults['icon_iphone_72']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).$iphone_72,'icon_iphone_72');
		}
		if (Configuration::get('STSN_ICON_IPHONE_114') != "") {
            $iphone_114 = Configuration::get('STSN_ICON_IPHONE_114');
		    $this->fields_form[18]['form']['input']['icon_iphone_114_field']['image'] = $this->getImageHtml(($iphone_114 != $this->defaults['icon_iphone_114']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).$iphone_114,'icon_iphone_114');
		}
		if (Configuration::get('STSN_ICON_IPHONE_144') != "") {
            $iphone_144 = Configuration::get('STSN_ICON_IPHONE_144');
		    $this->fields_form[18]['form']['input']['icon_iphone_144_field']['image'] = $this->getImageHtml(($iphone_144 != $this->defaults['icon_iphone_144']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).$iphone_144,'icon_iphone_114');
		}*/
        $iphone_icon_array = array('16', '32', '150', '180', '192', '512', 'svg');
        foreach($iphone_icon_array as $v)
        {
        ...
        if (Configuration::get('STSN_HEADER_BG_IMG') != "") {
            $this->fields_form[4]['form']['input']['header_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_HEADER_BG_IMG'), 'header_bg_img');
        }
		if (Configuration::get('STSN_HEADING_BG_IMAGE') != "") {
		    $this->fields_form[4]['form']['input']['heading_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_HEADING_BG_IMAGE'), 'heading_bg_image');
		}
		if (Configuration::get('STSN_BODY_BG_IMG') != "") {
		    $this->fields_form[6]['form']['input']['body_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_BODY_BG_IMG'),'body_bg_img');
		}
		if (Configuration::get('STSN_F_TOP_BG_IMG') != "") {
		    $this->fields_form[7]['form']['input']['f_top_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_F_TOP_BG_IMG'),'f_top_bg_img');
		}
		if (Configuration::get('STSN_FOOTER_BG_IMG') != "") {
		    $this->fields_form[8]['form']['input']['footer_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_FOOTER_BG_IMG'),'footer_bg_img');
		}
		if (Configuration::get('STSN_F_SECONDARY_BG_IMG') != "") {
		    $this->fields_form[9]['form']['input']['f_secondary_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_F_SECONDARY_BG_IMG'),'f_secondary_bg_img');
		}
        if (Configuration::get('STSN_F_INFO_BG_IMG') != "") {
            $this->fields_form[10]['form']['input']['f_info_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_F_INFO_BG_IMG'),'f_info_bg_img');
        } 
		if (Configuration::get('STSN_AUTH_BG_IMG') != "") {
		    $this->fields_form[62]['form']['input']['auth_bg_image_field']['image'] = $this->getImageHtml(__PS_BASE_URI__.'stupload/'.Configuration::get('STSN_AUTH_BG_IMG'),'auth_bg_img');
		} 
    ...
    public function writeCss()
    {
        ...
        */    
        ...
        if(Configuration::get('STSN_MENU_VER_TITLE_WIDTH'))
            $css .= '#st_ma_0, #st_mega_menu_wrap .stmenu_sub.stmenu_vertical{ width: '.Configuration::get('STSN_MENU_VER_TITLE_WIDTH').'px; }';
        ...
        if ($side_panel_heading = Configuration::get('STSN_SIDE_PANEL_HEADING'))
            $css .= '.st-menu-title, .st-menu-header .close_right_side{color:'.$side_panel_heading.';}';
        ...
        if(Configuration::get('STSN_PS_PAG_NAV_BG_HOVER'))  
            $css .= '.products_slider .swiper-pagination-bullet-active{ background-color: '.Configuration::get('STSN_PS_PAG_NAV_BG_HOVER').'; }';
       ...
       /* if($mob_cart_icon_border_color = Configuration::get('STSN_MOB_CART_ICON_BORDER_COLOR'))
        ...
        if($truncate_cate_desc_height_mobile = (int)Configuration::get('STSN_TRUNCATE_CATE_DESC_HEIGHT_MOBILE'))
            $css .='@media (max-width: 991px) {.category-description.truncate_cate_desc_1, .manufacturer-description.truncate_cate_desc_1, .category-description.truncate_cate_desc_2, .manufacturer-description.truncate_cate_desc_2{max-height:'.$truncate_cate_desc_height_mobile.'px;}.category-description.truncate_cate_desc_1 .st_read_more, .manufacturer-description.truncate_cate_desc_1 .st_read_more, .category-description.truncate_cate_desc_2 .st_read_more, .manufacturer-description.truncate_cate_desc_2 .st_read_more{height: calc(4000% - '.($truncate_cate_desc_height_mobile*39).'px);}}';
    ...
    public function getStBasicVals(){
        ...
        if($footer_img_link)
        {
           $footer_img_src = (strpos($footer_img_link, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : ($footer_img_link==$this->defaults["footer_img"]['val'] ? $this->_path : __PS_BASE_URI__.'stupload/' )).$footer_img_link;
           $footer_img_src = $this->context->link->getMediaLink($footer_img_src);
        }
        $retina_logo = Configuration::get('STSN_RETINA_LOGO');
        if($vals['smarty_val']['retina'] && $retina_logo)
        {
           $retina_logo_src = $this->context->link->getMediaLink((strpos($retina_logo, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : __PS_BASE_URI__.'stupload/').$retina_logo);
        }
        $mobile_logo = Configuration::get('STSN_MOBILE_LOGO');
        if($mobile_logo)
        {
           $mobile_logo_src = $this->context->link->getMediaLink((strpos($mobile_logo, '/img/cms/') === 0 ? rtrim(__PS_BASE_URI__, '/') : __PS_BASE_URI__.'stupload/').$mobile_logo);
        }
        ...
        $extra_smarty_val = array(
            ...
            /*'icon_iphone_57' => Configuration::get('STSN_ICON_IPHONE_57') ? $this->context->link->getMediaLink((Configuration::get('STSN_ICON_IPHONE_57') != $this->defaults['icon_iphone_57']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).Configuration::get('STSN_ICON_IPHONE_57')) : '',
            'icon_iphone_72' => Configuration::get('STSN_ICON_IPHONE_72') ? $this->context->link->getMediaLink((Configuration::get('STSN_ICON_IPHONE_72') != $this->defaults['icon_iphone_72']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).Configuration::get('STSN_ICON_IPHONE_72')) : '',
            'icon_iphone_114' => Configuration::get('STSN_ICON_IPHONE_114') ? $this->context->link->getMediaLink((Configuration::get('STSN_ICON_IPHONE_114') != $this->defaults['icon_iphone_114']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).Configuration::get('STSN_ICON_IPHONE_114')) : '',
            'icon_iphone_144' => Configuration::get('STSN_ICON_IPHONE_144') ? $this->context->link->getMediaLink((Configuration::get('STSN_ICON_IPHONE_144') != $this->defaults['icon_iphone_144']['val'] ? __PS_BASE_URI__.'stupload/' : $this->_path).Configuration::get('STSN_ICON_IPHONE_144')) : '',*/
        ...
        if(Configuration::get('STSN_ICON_IPHONE_150')){
            if (!file_exists(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/browserconfig.xml'))
                $this->generateBrowserConfig(time());
            $extra_smarty_val['browserconfig'] = $this->context->link->getMediaLink(__PS_BASE_URI__.'stupload/'.$this->name.'/'.$this->context->shop->id.'/browserconfig.xml');
        }
        if(Configuration::get('STSN_ICON_IPHONE_192') || Configuration::get('STSN_ICON_IPHONE_512')){
            if(!file_exists(_PS_ROOT_DIR_.'/stupload/'.$this->name.'/'.$this->context->shop->id.'/site.webmanifest'))
                $this->generateWebmanifest(time());
            $extra_smarty_val['site_webmanifest'] = __PS_BASE_URI__.'stupload/'.$this->name.'/'.$this->context->shop->id.'/site.webmanifest';
        }
    ...
    {
        $image = __PS_BASE_URI__.'stupload/'.$image;
    ...
    public function getFavicon($icon,$key){
        ...
        if($icon != $this->defaults[$key]['val'] && strpos($icon, 'img/') !== 0){
            if((strpos($icon, '/img/cms/') === 0 || strpos($icon, '/upload/stthemeeditor/') === 0)){
                ...
                $icons=__PS_BASE_URI__.'stupload/'.$icon;
            }
        }else{
