      ...
      <div class="product-manufacturer pro_extra_info flex_container">
          ...
          <div class="pro_extra_info_content flex_child">
            {if isset($product_manufacturer->id) && $product_manufacturer->active}
              <a {if $sttheme.google_rich_snippets} itemprop="brand" itemscope="" itemtype="https://schema.org/Brand" {/if} href="{$product_brand_url}" title="{l s='Click here to see all products of this brand' d='Shop.Theme.Panda'}" target="_top" class="pro_extra_info_brand">
                  {if $sttheme.google_rich_snippets}<meta itemprop="name" content="{$product_manufacturer->name}" />{/if}
                  {if in_array($sttheme.show_brand_logo, [2,4])}
                      <div>{$product_manufacturer->name}</div>
                  {/if}
                  {if in_array($sttheme.show_brand_logo, [1,3,5])}
